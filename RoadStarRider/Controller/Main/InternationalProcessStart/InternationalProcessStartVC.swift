...
class InternationalProcessStartVC: BaseViewController, Storyboarded {
    ...
    @IBAction func btnOkeyTapped(_ sender: Any) {
        ...
        if self.selectedAirpoart == nil {
            ...
            Toast.showError(message: "Kindly add flight no")
            return
        } else if self.txtFlightNum.text == "" {
            Toast.showError(message: "Kindly add flight no")
            return
        }
        self.theDelegate?.getData(airport: self.selectedAirpoart!, flight: txtFlightNum.text!)
        self.dismiss(animated: true, completion: nil)
    }
                ...
                case let .success(value):
                    if let valueData = value as? [[String:Any]] {
                        ...
                        valueData.forEach { (dics) in
                            ...
                            let a = AirpoartResponse(id: dics["id"] as? Int, ident: dics["ident"] as? String, name: dics["name"] as? String)
